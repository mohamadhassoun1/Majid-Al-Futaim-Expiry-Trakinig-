...
// If not set, default to empty string to use relative paths (handled by Vite proxy in dev, or same-origin in prod)
...
const apiRequest = async (method: 'GET' | 'POST' | 'PUT' | 'DELETE', path: string, body?: object) => {
    ...
    try {
        ...
        if (error instanceof TypeError && error.message === 'Failed to fetch') {
            throw new Error('Network error: Could not connect to the backend. Please ensure the server is running.');
        }
        throw error;
    }
};
