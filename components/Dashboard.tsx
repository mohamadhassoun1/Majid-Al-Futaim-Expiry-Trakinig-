...
const Dashboard: React.FC<DashboardProps> = ({ appData, currentUser, onNavigateToAdmin, onAddItem, onUpdateItem, onDeleteItem }) => {
  ...
  const handleAiQuerySubmit = async (e: React.FormEvent) => {
    ...
    // Handle offline mode gracefully - Removing explicit offline message
    if (currentUser.isDemo) {
        setIsThinking(true);
        // Simulate network delay
        setTimeout(() => {
             // More natural response without "OFFLINE" shouting
             setAiResponse(`Based on your local data, I can see that there are currently ${currentStoreItems.length} items in this store's inventory.`);
             setIsThinking(false);
        }, 1000);
        return;
    }
    ...
    try {
        ...
        // Use backend API to keep API Key secure
        const response: any = await apiPost('/ai/ask', {
            query: aiQuery,
            itemContext: itemContext || 'No items in this store.',
            systemInstruction: systemInstruction,
            credential: currentUser.credential // Needed for auth middleware
        });

        setAiResponse(response.text);
    } catch (error) {
        console.error("AI Error:", error);
        setAiResponse('Sorry, an error occurred while processing your request. The AI service might be unavailable.');
    } finally {
        setIsThinking(false);
    }
  };
